...
\a. The \textsc{noun}s each were \textsc{adjective}. \label{bnc-each}
\b. The \textsc{noun}s together were \textsc{adjective}. \label{bnc-together}


Participants were presented with 30 sentences; 15 of these sentences were naturally-occurring examples from the original set of the 40 most frequent instances. The other 15 sentences were constructed by randomly pairing subjects and predicates from the set of 40 to create novel sentences.

Each sentence was presented as an utterance by a speaker, and participants were tasked with determining what the speaker meant. Participants rated two possible possible paraphrases on a slide scale. The paraphrases were either distributive (with ``each'', \ref{bnc-each}) or collective (with post-nominal ``together'', \ref{bnc-together}). Scale endpoints corresponded to whether the paraphrase was ``definitely'' or ``definitely not'' what the speaker intended by his or her utterance. Before rating its paraphrases, participants judged whether the utterance made sense (choosing between "Yes" and "No"). In the analyses reported below, we only look at responses to utterances that participants said made sense.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=.985\linewidth]{plots/noun_plot.eps}
	%\vspace{0pt}
	\caption{Collective paraphrase endorsement rates for different subject nouns.} \label{nouncoll}
\end{figure}
...
As with Expt.~2a, we only look at responses to utterances that participants indicated made sense to them.
...
If contextual predictability influences the viability of collective interpretations, we should find that noun-predicate pairs that plausibly implicate a limited set of collective properties---as was the case with \emph{small numbers} in Expt.~2a---yield greater rates of collective interpretations. For example, with \emph{big waves}, the collective property named most likely targets just a single dimension of the waves: their height. We would therefore expect greater rates of collective interpretations for \emph{big waves}.
