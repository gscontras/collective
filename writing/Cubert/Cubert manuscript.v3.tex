...
By Bayes' rule:
$$P_{L_{1}}(s,v,\theta_d,\theta_c|u,a) \propto P_{S_{1}}(u|o(a,s),v,\theta_d,\theta_c) \cdot P(s) \cdot P(v)$$
Here $o(a,s)$ is a deterministic function that reads off the observation the speaker would have had given an access mode and a hypothesized true state. We have assumed uninformative, uniform priors on the threshold variables.
...
\ndg{this section can be simplified somewhat, since i included some additional details (eg access, degrees) in the above. why separate quantitative and qualitative results? we could simply use the fit values and generate all the same plots?}
