...
Fig.\ \ref{resultsexpt4} displays the collective paraphrase endorsement ratings averaged across all trials with bootstrapped 95\% confidence intervals; higher rates of endorsement for the collective paraphrase signal greater rates of collective interpretation. We grouped data by the dimension modified as well as its valence.
...
For each dimension, we fit a linear regression model predicting collective endorsement ratings by \textsc{context} (``random'' vs.~``regular'') and \textsc{trial} order; for those dimensions with both positive and negative adjectives (i.e., height, size, and width), models additionally included the \textsc{valence} predictor and the interaction between \textsc{context} and \textsc{valence}.\footnote{All reported results hold if instead we look at normalized collective paraphrase endorsement ratings (i.e., the difference between collective and distributive endorsements for each trial), given the strong negative correlation between the two endorsement ratings ($r=-0.49$).} The full regression models appear in \ref{expt4results}.

We begin with a look at the dimensions with only positive-valence instances. For \textbf{capacity} adjectives, the model found a marginally significant effect of \textsc{context} ($\beta$ = 0.16, $SE$ = 0.08, $t$ = 1.95, p $<$ 0.06), such that regular contexts tended to have more collective interpretations than did random contexts. There was no significant effect of \textsc{context} for \textbf{depth} ($\beta$ = 0.07, $SE$ = 0.09 $t$ = 0.85, p $<$ 0.40), \textbf{length} ($\beta$ = 0.01, $SE$ = 0.06, $t$ = 0.15, p $<$ 0.89), or \textbf{weight} ($\beta$ = -0.04, $SE$ = 0.07, $t$ = -0.51, p $<$ 0.62) adjectives.

We turn next to those dimensions with both negative- and positive-valence instances. For \textbf{height} adjectives, the model found a main effect of valence ($\beta$ = 0.28, $SE$ = 0.04, $t$ = 7.82, p $<$ 0.01), such that positive-valence height adjectives received more collective interpretations than negative-valence height adjectives. The model also found a marginally significant interaction between \textsc{context} and \textsc{valence} ($\beta$ = 0.13, $SE$ = 0.07, $t$ = 1.82, p $<$ 0.07). A post-hoc analysis revealed that this interaction was driven by a significant effect of \textsc{context} within the positive-valence height adjectives ($\beta$ = 0.12, $SE$ = 0.06, $t$ = 2.11, p $<$ 0.05): regular contexts yielded more collective interpretations. For \textbf{size} adjectives, the model found a main effect of valence ($\beta$ = 0.20, $SE$ = 0.03, $t$ = 7.60, p $<$ 0.01), such that positive-valence height adjectives received more collective interpretations than negative-valence height adjectives. The model also found a significant interaction between \textsc{context} and \textsc{valence} ($\beta$ = 0.13, $SE$ = 0.05, $t$ = 1.82, p $<$ 0.05). A post-hoc analysis revealed that this interaction was driven by a significant effect of \textsc{context} within the positive-valence size adjectives ($\beta$ = 0.10, $SE$ = 0.04, $t$ = 2.21, p $<$ 0.05): regular contexts yielded more collective interpretations.
For \textbf{width} adjectives, the model found a significant interaction between \textsc{context} and \textsc{valence} ($\beta$ = -0.14, $SE$ = 0.07, $t$ = -2.02, p $<$ 0.05). A post-hoc analysis revealed that this interaction was driven by a significant effect of \textsc{context} within the \emph{negative}-valence width adjectives ($\beta$ = 0.13, $SE$ = 0.05, $t$ = 2.66, p $<$ 0.01): regular contexts yielded more collective interpretations.
...
Tables \ref{expt4analysisa} and \ref{expt4analysisa} present model coefficients for the full linear regression models used in the analysis of Expt.~4. For each dimension, the models predict collective endorsement ratings from fixed effects for context and trial; for those dimensions with both positive- and negative-valence adjectives, the models also included fixed effects of valence and the interaction between valence and context. The fixed effects predictors were centered before analysis.

\begin{table}[htb] 
	\renewcommand\thetable{D.1}
	\centering \caption{Full linear regression models from the positive-valence dimension analyses of Expt.~4.} \label{expt4analysisa}
	\begin{tabular}{lrrrr}\toprule
		&	Coef $\beta$	&	SE($\beta$)	&	\multicolumn{1}{c}{ \textbf{t}}	&	\multicolumn{1}{c}{$p$}\\ \midrule
		\emph{\textbf{capacity}} \\
...
\begin{table}[htb] 
	\renewcommand\thetable{D.1}
	\centering \caption{Full linear regression models from the multi-valence dimension analyses of Expt.~4.} \label{expt4analysisb}
	\begin{tabular}{lrrrr}\toprule
		&	Coef $\beta$	&	SE($\beta$)	&	\multicolumn{1}{c}{ \textbf{t}}	&	\multicolumn{1}{c}{$p$}\\ \midrule
		\emph{\textbf{height}}\\
