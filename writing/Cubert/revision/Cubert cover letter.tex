...
The reviewer raised the issue of how the likely truth of a given interpretation might affect its probability: interpretations that are more obviously true should be more probable. We explore this idea in our new experiment (\emph{Expt.~4: Generalizing our findings}; cf.~our response to the Editor's point 1 above), and find that an interpretation's probability of being judged true affects that interpretation's overall probability. We expand on this point below in our responses to Reviewer 1's point 1 and Reviewer 3's point 14.
...
We have added the requested information, as well as gender self-reports, for each of our experiments.
...
Our experiments identified roles for contextual predictability and speaker access to knowledge in the resolution of ambiguity in plural predication; we highlight both factors throughout the paper. By more clearly motivating the speaker knowledge experimental manipulation (cf.~our response to Reviewer 2's point 22 below), we further highlight the role of the speaker's epistemic state.
...
The relative availability of collective interpretations for \emph{big} and \emph{tall} (and, indeed, \emph{heavy}) depends on the contextual predictability of the collective properties these predicates name. \emph{Tall}'s collective property is more predictable than \emph{big}'s, hence the introspective difference. One of the main points of our paper is exactly this: collective interpretations are available to the extent that they are potentially informative, and informativity depends crucially on the contextual predictability of the collective properties at play.
...
We treat the ambiguity at the utterance level, so that the full sentence gets disambiguated.
...
We have softened the transition.
...
We hope that our revisions and responses to the reviewer's comments will help the reviewer more clearly see through to our conclusions.
...
There isn't yet a sense of what counts as high or low for the optimality parameter $\alpha$ in RSA models. In an attempt to keep our paper at a manageable length, rather than discussing alternative parameter settings in detail with accompanying plots, we chose to link to the full model, where the interested reader can manipulate the parameters and assumptions on their own.
